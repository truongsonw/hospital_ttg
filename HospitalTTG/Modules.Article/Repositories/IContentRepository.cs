...
internal interface IContentRepository
{
    ...
    Task<(IReadOnlyList<Content> Items, int Total)> GetPagedAsync(string? type, string? categorySlug, byte? status, int page, int pageSize, CancellationToken ct = default);
